...
public class RSAUtil {
	...
	private static byte[] rsaSplitCodec(Cipher cipher, int opmode,
										byte[] datas, int keySize) throws Exception {
		int maxBlock = 0;
	 ...
	 * 使用RSA验签
	 */
	public static boolean checkSignWithRSA(Map<String,String> paramsMap, String publicKey, String sign) throws NoSuchAlgorithmException, InvalidKeyException, InvalidKeySpecException, SignatureException, UnsupportedEncodingException {
		Map<String, String> map=new HashMap<>();
		map.putAll(paramsMap);
		if(map.containsKey("sign")){
			map.remove("sign");
		}
		String content = formatSignContent(map);
		Signature signature = Signature.getInstance("SHA1WithRSA");
